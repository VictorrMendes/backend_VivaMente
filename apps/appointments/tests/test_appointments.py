...
class AvailabilitySlotTests(AuthenticatedAPITestCase):
    def setUp(self):
...
class AppointmentOwnershipTests(AuthenticatedAPITestCase):
    def setUp(self):
        self.therapist_a = User.objects.create(firebase_uid="ther-a", email="a@teste.com", role=User.THERAPIST)
        self.therapist_b = User.objects.create(firebase_uid="ther-b", email="b@teste.com", role=User.THERAPIST)
        self.prof_a = Professional.objects.create(user=self.therapist_a, slug="terapeuta-a", full_name="A")
...
class AppointmentTransitionTests(AuthenticatedAPITestCase):
    ...
    def _transition(self, action):
        return self.client.patch(f"/api/v1/appointments/{self.appointment.id}/{action}")
    ...
    def test_confirmed_can_be_cancelled(self):
        ...
        response = self._transition("cancel")
        self.assertEqual(response.status_code, 200)
